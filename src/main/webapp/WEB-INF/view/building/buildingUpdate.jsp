 ...
 function error_numbr(str, field){ 
  ...
  field.focus(); 
	 ...
	 /*유효성 관련 스크립트*/
	   ...
	   } else if( $("#sImg1").val()==''&& $('#sImg1File1').val()==''){
	      alert("대표이미지를 확인하세요~><");
	      document.getElementById('info').focus();
	    } else if($('#zipcode').val()==''){
	      alert("우편번호를 확인하세요~><");
...
<body>
<div>
<form:form name="uf" modelAttribute="building" method="post" action="buildingUpdateReg.sms" enctype="multipart/form-data">
...
<input type="hidden" id="sImg2Name" name="sImg2Name" value="${building.sImg2Name}">
  ...
  <tr><td colspan="2" align="center">공간정보 수정</td></tr>
    ...
    <tr><td>대표이미지</td>
       ...
       <input type="file" id="sImg1File1" name="sImg1File" accept="image/*"/>
       </td></tr>
    ...
    <tr>
		...
		<tr>
			<td>우편번호</td><td><input type="text" name="sAddress" id="zipcode" class="postcodify_postcode5" value="${address1}" readonly/>
				<input type="button" id="postcodify_search_button" value="검색"></td></tr>
		<tr>
			<td>도로명 주소</td><td><input type="text" id="asdf" name="sAddress" class="postcodify_address" value="${address2}" readonly/></td></tr>
		<tr>
    ...
    <tr><td align="left">공간 연락처</td>
		<td>
		<input type="text" name="sTel" value="${building.sTel}" > 
		</td></tr>
    ...
    <input type="button" value="공간수정" onclick="checku()">
    <input type="button" value="수정취소" onclick="javascript:history.back()"></td></tr>
  </table>
<!-- 지도관련 스크립트 -->
<script>
